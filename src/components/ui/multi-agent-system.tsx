...
export function AgentTeamOverview({ agents, onViewLiveFeed, onAdjustStrategy, className }: AgentTeamOverviewProps) {
    ...
    >
      <Card className="w-full max-w-2xl" variant="glass" blur="lg">
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-base">Multiple specialized agents working together</CardDescription>
        </CardHeader>

        <CardContent className="space-y-6">
          <div className="space-y-4">
            {agents.map((agent) => (
              <Card key={agent.id} className="overflow-hidden" variant="solid">
                <CardContent className="p-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {agent.stats && <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{agent.stats}</p>}
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            {onViewLiveFeed && (
              <Button onClick={onViewLiveFeed} className="flex-1" color="primary">
                View Live Feed
              </Button>
            )}

            {onAdjustStrategy && (
              <Button onClick={onAdjustStrategy} variant="outline" className="flex-1">
                Adjust Strategy
              </Button>
            )}
          </div>
...
export function LiveAgentCollaboration({ steps, stats, onPause, className }: LiveAgentCollaborationProps) {
  return (
    <div
      className={cn(
        'min-h-screen bg-gradient-to-br from-primary-50 to-secondary-50 dark:from-gray-950 dark:to-gray-900 flex items-center justify-center p-4',
