...
export function NotificationCard({
  ...
  const getTypeIcon = () => {
    ...
    switch (type) {
      ...
      case 'feature': return <Bell className="w-5 h-5" />
  ...
  const getTypeColors = () => {
    switch (type) {
      ...
      case 'feature': return 'border-purple-200 bg-purple-50 dark:border-purple-800 dark:bg-purple-950/20'
    ...
    <Card className={cn(getTypeColors(), className)} variant="solid">
      <CardContent className="p-4">
        <div className="flex items-start gap-3">
          <div className={cn("flex-shrink-0 mt-0.5", getIconColors())}>
            {getTypeIcon()}
          </div>
...
export function VoiceAgentPrompt({ 
  missedCalls, 
  onSetupNow, 
  onLater, 
  className 
}: VoiceAgentPromptProps) {
  return (
    <NotificationCard
      type="feature"
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h4 className="font-medium text-red-900 dark:text-red-200 mb-1">
              ⚠️ Suspicious Job Email Detected
            </h4>
