        ...
        <CardHeader className="pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>AI managing your job applications everywhere</CardDescription>
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="text-sm text-gray-600 dark:text-gray-400">{platform.applied} applied</div>
                  </div>
                </div>

                {platform.responseRate && (
                  <Badge className="mr-2 bg-green-100 text-green-800 border-green-200">{platform.responseRate}%</Badge>
                )}

                <div className="flex items-center gap-1.5">
                  {getStatusIcon(platform.status)}
                  <span className="text-sm hidden sm:inline">{getStatusText(platform.status)}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Card variant="solid" className="bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800">
              <CardContent className="p-4">
                <div className="flex flex-col items-center text-center">
                  <p className="text-sm text-blue-700 dark:text-blue-300">Duplicates prevented</p>
                  <p className="text-xl font-bold text-blue-800 dark:text-blue-200">{duplicatesPrevented}</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card variant="solid" className="bg-green-50 dark:bg-green-950/20 border-green-200 dark:border-green-800">
              <CardContent className="p-4">
                <div className="flex flex-col items-center text-center">
                  <p className="text-sm text-green-700 dark:text-green-300">Total unique applications</p>
                  <p className="text-xl font-bold text-green-800 dark:text-green-200">{totalApplications}</p>
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          {bestPlatform && (
            <div className="p-4 rounded-lg bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800">
              <h3 className="font-medium text-green-800 dark:text-green-300 mb-1">Best performing platform:</h3>
              <div className="flex items-center justify-between">
                <span className="text-green-700 dark:text-green-400">{bestPlatform.name}</span>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            {onAddPlatform && (
              <Button onClick={onAddPlatform} variant="outline" className="flex-1 gap-2">
