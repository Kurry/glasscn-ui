...
import { cn, createSurface } from '@/lib/utils'
...
export const Surface = React.forwardRef<HTMLDivElement, SurfaceProps>(
  (
    { className, variant = 'solid', elevated = false, hasBorder = false, innerGlow = false, children, ...props },
    ref,
  ) => {
    return (
      <div ref={ref} className={cn(createSurface(variant, elevated, hasBorder, innerGlow), className)} {...props}>
        {children}
      </div>
    )
  },
...
export const GlassPanel = React.forwardRef<HTMLDivElement, GlassPanelProps>(
  ({ className, elevated = false, innerGlow = false, children, ...props }, ref) => {
    return (
      <Surface
        ref={ref}
        variant="glass"
  ...
  'inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-white transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-600 hover:-translate-y-0.5 active:scale-98 disabled:pointer-events-none disabled:opacity-50',
  {
    variants: {
      variant: {
        ...
        outline:
          'border border-gray-300 bg-white hover:bg-gray-100 text-gray-900 dark:border-gray-700 dark:bg-gray-950 dark:hover:bg-gray-800 dark:text-gray-50 backdrop-blur-sm',
...
export const DepthContainer = ({ level, className, children, ...props }: DepthContainerProps) => {
  return (
    <div className={cn(`z-${level}`, className)} {...props}>
      {children}
    </div>
  )
}
...
export const GlassRing = ({ size, blur = 2, glow = false, innerGlow = false, className, ...props }: GlassRingProps) => {
  return (
    <div
      className={cn(
    ...
    <div className="absolute inset-0 -z-10 overflow-hidden pointer-events-none">
      ...
      <div className="absolute left-1/2 top-0 -translate-x-1/2 contain-paint">
        ...
        <GlassRing
          size={30}
          blur={3}
          className="absolute inset-0 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 opacity-40"
        />
        <GlassRing
          size={20}
          blur={2}
          glow
          className="absolute inset-0 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 opacity-60"
        />
        <GlassRing
          size={10}
          blur={1}
          glow
          innerGlow
          className="absolute inset-0 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 opacity-80"
        />
      </div>
...
export function GlassNavbar({ scrolled = false, className, children, ...props }: GlassNavbarProps) {
  return (
    <GlassPanel
      elevated={scrolled}
      className={cn('sticky top-0 w-full transition-all duration-300', scrolled ? 'py-2' : 'py-4', className)}
      {...props}
    >
      {children}
    </GlassPanel>
...
export function MaterialCard({
  ...
  return (
    <Surface variant={variant} elevated={elevated} className={cn('p-6', className)} {...props}>
      {children}
    </Surface>
  )
}
