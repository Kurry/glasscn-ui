...
export function InitialDashboardGeneration({ 
  userName,
  onProfileAnalyzed,
  className 
  ...
  return (
    <div className={cn("min-h-screen bg-gradient-to-br from-primary-50 to-secondary-50 dark:from-gray-950 dark:to-gray-900 p-4", className)}>
...
export function DynamicResumeAnalysis({
  findings,
  progress,
  onAnalysisComplete,
  className
            ...
            <Card key={match.id} variant="glass" blur="sm">
              <CardContent className="p-4">
                <div className="space-y-3">
                  <div>
                    <h3 className="text-lg font-bold">{match.company} - {match.position}</h3>
                  ...
                  <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
                    <div className="flex-1">
                      📄 Resume tailored {match.isResumeReady ? '✓' : '...'} | 
                      Cover letter ready {match.isCoverLetterReady ? '✓' : '...'}
                    </div>
                    <div className="flex gap-2">
                      {onApply && (
                        <Button 
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold">
              🤖 AI Agents Active | 
              {activeApplications.length} Applying | 
              {completedApplications.length} Complete | 
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <div className="text-sm text-gray-600 dark:text-gray-400">
                      Step {app.timeline.filter(t => t.isCompleted).length + (app.timeline.some(t => t.isActive) ? 1 : 0)}/{app.timeline.length} - {app.currentStep}
                    </div>
                ...
                <div key={app.id} className="flex items-center justify-between p-3 bg-white/50 dark:bg-gray-800/50 rounded-lg border">
                  ...
                  {onViewRecording && (
                    <Button 
                      size="sm" 
                      variant="ghost"
                      onClick={() => onViewRecording(app.id)}
                    >
                      View Recording
                    </Button>
                  )}
            ...
            {resumeVariants.map((variant) => (
              <Card 
                ...
                className={cn(
                  ...
                  variant.isDefault && "ring-1 ring-primary-500"
                )}
              >
          ...
          <div className="mb-6">
            ...
            <Card variant="glass" blur="sm">
              <CardContent className="p-4">
                <div className="space-y-3">
                  {completedApplications.map((app) => (
                    <div key={app.id} className="flex items-center justify-between py-2 border-b border-gray-200 dark:border-gray-700 last:border-0">
                      <div>
                        <div className="font-medium">{app.company} - {app.position}</div>
                        {app.nextStep && (
                          <div className="text-sm text-gray-600 dark:text-gray-400">
      ...
      <div className="p-4">
        {currentState === 'initial' && (
          <InitialDashboardGeneration 
            userName="John Smith" 
            onProfileAnalyzed={() => handleStageChange('empty')}
          />
        )}
