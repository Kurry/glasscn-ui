...
import { AlertTriangle, Check, CheckCircle, FileX, Shield, Sparkles } from 'lucide-react'
...
export function ApplicationQualityControl({
  ...
  const getSeverityBadge = (severity: PotentialIssue['severity']) => {
    switch (severity) {
      case 'high': return <Badge className="bg-red-100 text-red-800 border-red-200">High</Badge>
        ...
        <CardHeader className="pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>
                Before submitting to {companyName}
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          <div className="space-y-4">
            {potentialIssues.map((issue) => (
              <Card key={issue.id} variant="solid" className="bg-white/80 dark:bg-gray-800/80 border-gray-200 dark:border-gray-700">
          ...
          <Card variant="solid" className={cn(
            ...
            isPassed ? "bg-green-50 dark:bg-green-950 border-green-200 dark:border-green-800" 
